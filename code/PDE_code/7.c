// complie: gcc -o tx 7.c -lrt -lpthread
...
int main(int argc, char **argv) {
    ...
    DelayTimeStamp *timestamps = malloc(NUM_ITERATIONS * sizeof(DelayTimeStamp));
    if (timestamps == NULL) {
    ...
    for (int i = 0; i < NUM_ITERATIONS; ++i) {
        ...
        timestamps[i] = delay_busy_wait(SLEEP_TIME);

        long loop_duration = (timestamps[i].end.tv_sec - timestamps[i].start.tv_sec) * 1000000 + (timestamps[i].end.tv_nsec - timestamps[i].start.tv_nsec) / 1000;
