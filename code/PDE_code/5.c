...
int main(int argc, char **argv) {
    ...
    int threshold_time = (int)(SLEEP_TIME * 1.01);
    ...
    long *loop_times = malloc(NUM_ITERATIONS * sizeof(long));
    if (loop_times == NULL) {
        perror("Error allocating memory for loop times");
        free(data_array);
        exit(EXIT_FAILURE);
    }
    ...
    // Write loop times to a file
    FILE *output_file = fopen("loop_times.csv", "w");
    if (output_file == NULL) {
    ...
    fprintf(output_file, "Iteration,Duration (us)\n");
    for (int i = 0; i < NUM_ITERATIONS; ++i) {
